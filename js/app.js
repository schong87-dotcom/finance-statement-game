...
(function(){
  ...
  // 구글 로그인 버튼 — Supabase OAuth로 구글에 다녀온다
  function setupGoogleLogin() {
    ...
    btnHost.innerHTML = `
      <button type="button" id="btn-google"
              class="w-full max-w-[300px] flex items-center justify-center gap-2.5 bg-white border border-gray-300 hover:bg-gray-50 active:bg-gray-100 text-gray-700 font-semibold text-sm py-2.5 rounded-full shadow-sm transition-all">
        <svg class="w-[18px] h-[18px]" viewBox="0 0 18 18" aria-hidden="true">
    ...
    document.getElementById('btn-google').addEventListener('click', async () => {
      ...
      // 성공 시에는 구글 페이지로 이동하므로 여기서 할 일이 없다
    });
  }
